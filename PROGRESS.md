...
- **Local/Git:** branch `main`; local `HEAD` == `origin/main` == `ff8f5b71ee356bf3397fb92ebdd1cd31a81c7415` (`feat: preserve study context across word lookup`). Untracked historical docs `docs/ARCHITECTURE_RECONSTRUCTION_V1.md` and `docs/STORY_MINING_V1.md` remain preserved and untouched.
...
- **Accepted milestones:** `AI_AUTH_COST_BOUNDARY_V1`, dictionary reference consistency, the Option B/Supadata Vercel-first non-ASR subtitle boundary, and the YouTube route-leave regression remain accepted at their recorded evidence levels. Lemma provenance is **STALE / NOT A BUG**.
- **Study context continuity:** `STUDY_CONTEXT_CONTINUITY_V1` is **LOCAL VERIFIED** and deployed in `ff8f5b7`, with convergence 4/4, `WordDictionaryPopup` 18/18, `tsc` PASS, targeted ESLint 0 errors, diff-check PASS, and real-learning-flow 1/1 PASS. Production deployment is proven READY, but behavioral smoke remains **UNVERIFIED / PARTIAL** because tooling could not safely seed a synthetic session or install network interception; no Production behavioral acceptance is claimed.
