...
- **Local/Git:** branch `main`; local `HEAD` == `origin/main` == `31d5f373f0647f105165c993bc473dfb160a8f20` (`31d5f37`, `fix(study): keep the 2x speed preset first-level`), 0/0 ahead/behind; CI is **GREEN** on this commit (run `34795278184`). Release train this cycle: `8be5867` (`feat(study): move timer, CEFR level and reload into a settings menu`, 5 files) + `31d5f37` (1 file). Untracked historical docs `docs/ARCHITECTURE_RECONSTRUCTION_V1.md` and `docs/STORY_MINING_V1.md` remain preserved and untouched (now also listed in `.gitignore` so they cannot be staged accidentally).
- **Production:** serves the build of `31d5f37`, verified behaviourally by bundle markers (no Vercel control-plane read this cycle) **and additionally by a fresh-guest behavioural Production smoke on 2026-09-14** (45/45 checks, `/api/ai` = 0). AI provider configuration was **not re-verified** this cycle; the last recorded state is Gemini (`AI_PROVIDER=gemini`) manually confirmed in the Vercel Dashboard for Production scope on 2026-09-13, and no secret value is recorded.
