...
- **Local/Git:** branch `main`; local `HEAD` and `origin/main` were synchronized (0/0 ahead/behind) at the 2026-09-13 verification; the latest verified commit at that check was `eea6639` (`Add learning flow lifecycle regressions`). Exact SHAs belong to the dated TEST_REPORT evidence sections, not this snapshot. Untracked historical docs `docs/ARCHITECTURE_RECONSTRUCTION_V1.md` and `docs/STORY_MINING_V1.md` remain preserved and outside every milestone.
- **Production:** AI auth cost boundary is **PRODUCTION ACCEPTED** (2026-09-13). Anonymous `/api/ai` → 401 before any body validation or provider fetch; fresh Guest learning journeys save words/sentences with `/api/ai` attempts = 0 (request-level evidence); an authenticated Production session completed the full positive path (browser → `getIdToken()` Bearer → server JWKS verify → real DeepSeek 200 → usable UI result) for both transcript Analyze and vocabulary AI enrichment. See `TEST_REPORT.md` 2026-09-13 for the dated evidence and call counts.
- **Accepted milestones:** dictionary reference consistency (2026-09-12), the Option B/Supadata Vercel-first non-ASR reliability milestone, `GUEST_AI_COST_BOUNDARY_V1` (deployed `391a08e`+`962f613`, guest negative boundary production-accepted 2026-09-13), and `AI_AUTH_COST_BOUNDARY_V1` (authenticated positive path production-accepted 2026-09-13). Explicit ASR remains the separate Worker opt-in boundary.
- **Current engineering status:** no proven high-value engineering bug is active. One transient observation from the 2026-09-13 authenticated smoke: the first Analyze call returned 200 with an empty content stream and the designed local fallback engaged honestly; an immediate retry succeeded — recorded as a backlog observation (fallback copy says "unavailable" for a transient empty stream), not an active defect.
...
- **Current work queue (in order):** `DEEPSEEK_V4_MODEL_REFRESH_V1` (research-first current-model/thinking-mode verification), lemma-provenance bounded verification (classification first, fix only if confirmed), then `REAL_LEARNING_SESSION_UX_V2` product selection. None of these changes release state.
