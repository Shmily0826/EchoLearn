...
- **Local/Git:** branch `main`; local `HEAD` == `origin/main` == `3c5fb62e83ae37792a596d5bfb3df73805cb15e4` (`Fix pre-existing CI lint and duplicate-save E2E regressions`), 0/0 ahead/behind; CI is **GREEN** again on this commit after two pre-existing failures were fixed. The milestone commit before it is `2b151f0` (`Close dictionary popup and keep the source sentence visible`). Untracked historical docs `docs/ARCHITECTURE_RECONSTRUCTION_V1.md` and `docs/STORY_MINING_V1.md` remain preserved and untouched.
- **Production:** current primary AI provider is Gemini (`AI_PROVIDER=gemini`), manually verified in the Vercel Dashboard for Production scope on 2026-09-13. `GEMINI_MODEL` and `GEMINI_API_KEY` are configured in Production; no secret value is recorded. Latest proven Production deployment `ff8f5b7` is **READY**.
...
- **Current engineering status:** no proven high-value engineering bug is active. The YouTube route-leave deterministic regression already exists and is not a next item.
- **Current product direction:** `REAL_LEARNING_SESSION_UX_V2` is the next substantive product-direction candidate, not yet accepted implementation scope. Other evidence-dependent follow-up remains limited to Supadata cost/billing review, Invidious/Piped health and pruning, and low-priority consolidation leftovers.
