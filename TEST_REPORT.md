...
- **2026-09-22 - `ECHO-20260922-DICT-BACKFILL-PACING-V1` dictionary backfill self-throttle + Bilibili caption monitor control:** LOCAL VERIFIED. `tsc -b` clean, lint 0 errors / 12 baseline warnings, Vitest **724/724** (+6), build PASS, dictionary E2E **5/5**, live `node scripts/health-check.mjs` → `All 7 checks passed (2 known-degraded warnings)`. Each guard falsified separately. No Production run of the throttled backfill is claimed. Evidence below.
...
- **Result: LOCAL VERIFIED.** Four files changed: `src/services/dictionaryService.ts`, `src/pages/VocabularyPage.tsx`, `src/i18n/translations.ts` (one progress key, en+zh), `scripts/health-check.mjs`, plus tests.
...
- **Not covered.** No Production run of the throttled backfill against a real large library (the pacing and cooldown are proven against mocks, not against the live edge limiter), no authenticated or real-device pass, no Firestore emulator run (no rules or data shape changed), and the new Bilibili control has never observed a PASS.
