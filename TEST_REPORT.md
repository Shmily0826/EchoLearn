...
- **Not covered / limits:** the seek button has no E2E assertion (the AI suite mocks `ai`, and the real-sample transcript is only reachable in a dogfood flow); the matcher is covered at unit level against the real fixture, which is the evidence that matters for alignment quality. Alignment was not re-verified against a second real transcript. `ai-quality-rubric.mjs` grounding scores (41%/47%/50%) remain deliberately out of CI. **Production serves the code but has NOT been behaviourally smoked** — the feature is behind sign-in and requires one real `/api/ai` call to reach, so the deployed bundle check above is a deployment/asset fact, not proof that the seek control behaves correctly on Production.
