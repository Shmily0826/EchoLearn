...
- **2026-09-22 - `ECHO-20260922-PROTOTYPE-KEY-CRASH-P0` Study route crash on a subtitle containing "constructor":** reproduced against live Production, fixed locally, each guard falsified. `tsc -b` clean, lint 0 errors / 12 baseline warnings, Vitest **731/731** (+7), build PASS, local-audio + learner-friction E2E **13/13**. Release and post-release Production re-check recorded below.
...
- **Result: reproduced on Production, fixed, falsified; release recorded below.** Files: `src/utils/lemmatizer.ts`, `src/services/dictionaryService.ts`, `src/services/localAudio.ts`, `src/utils/__tests__/lemmatizer.test.ts`, `e2e/local-audio.spec.ts`.
...
- **Release.** Pending at the time of writing: the branch carries this fix, and the Production re-check (re-running the same repro against `echo-learn.uk` after the deploy) is recorded here: PENDING.
