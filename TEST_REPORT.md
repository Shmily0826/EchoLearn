...
- **CI: currently RED.** The latest run is `35086564253` on `08d630b` (2026-09-16T10:44:01Z, 3m55s): the `test` job passed, the `e2e` job failed with **1 failed / 47 passed** — `e2e/media-sync.spec.ts:120` (`explicit transcript selection temporarily overrides playback-derived context`) expected the context bar to follow playback to `It's been great` but it stayed on `Good morning`. The cause was the test's own click target, not product code; a fix is committed locally (see the 2026-09-16 CI-failure section) and **no green CI run has been observed on it yet**. The last observed green run remains `34795278184` on `31d5f37` (46 passed / 0 failed, 2.4m).
...
- **Not covered:** no green CI run yet (the fix is local and unpushed at the time of writing). Only the `desktop-chromium` project was exercised locally; `mobile-chromium` / `mobile-webkit` were not run, and no full-suite local pass is claimed. The remaining 47 CI cases were already green on `08d630b` and were not re-run locally. Whether the same "default centre click on a row" pattern exists elsewhere was checked: `media-sync.spec.ts:136` clicks a word deliberately (correct as-is), and no other spec clicks a `[data-transcript-line]` row.
