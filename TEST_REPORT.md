...
- **AI auth cost boundary:** `AI_AUTH_COST_BOUNDARY_V1` is **PRODUCTION ACCEPTED** (2026-09-13). All three validation layers hold on `https://echo-learn.uk`: anonymous `/api/ai` → 401 before body validation; fresh Guest journeys save with `/api/ai` attempts = 0; authenticated positive path (Bearer token → server JWKS verify → real DeepSeek 200 → usable UI result) verified for both transcript Analyze and vocabulary AI enrichment. Dated evidence: "2026-09-13 - AI_AUTH_COST_BOUNDARY_V1 production acceptance" below.
- **Production accepted:** Dictionary reference consistency is **PRODUCTION ACCEPTED** (2026-09-12, deployed exact SHA recorded in the dated section). The acceptance smoke used local `/api/dictionary` fixtures; paid/provider outbound traffic was `0`.
- **Accepted subtitle milestones:** Worker-only production acceptance and the later Option B/Supadata Vercel-first non-ASR reliability milestones are accepted release evidence. They are not current `NEXT` items; explicit ASR remains the separate Worker opt-in boundary.
- **Real learning flow (2026-09-13):** the deterministic guest journey E2E (`e2e/real-learning-flow-journey.spec.ts`, committed in `eea6639`) passes with hard network invariants (`/api/ai` = 0, no provider egress); the same journey passed as a Production dogfood with analytics suppression verified. Golden-path and audio-route-lifecycle suites pass. Dated evidence: "2026-09-13 - AI_AUTH_COST_BOUNDARY_V1 production acceptance" below.
- **Current revalidation of the four older friction items:**
  - Audio silent-prewarm: **STALE / NOT ACTIVE**.
  - Guest onboarding versus AI: **RESOLVED / PREVENTED** (guest save path verified end-to-end on Production 2026-09-13).
  - Failed/empty transcript zero-line session: **RESOLVED / PREVENTED**.
  - `undefined.toLowerCase`: **INSUFFICIENT EVIDENCE**.
- **Current open evidence-dependent work only:** Supadata cost/billing review, Invidious/Piped health and pruning, and low-priority consolidation leftovers. `DEEPSEEK_V4_MODEL_REFRESH_V1` is **LOCAL VERIFIED** with Production verification pending an authorized deploy; lemma-provenance bounded verification remains queued, and `REAL_LEARNING_SESSION_UX_V2` remains a future product-selection direction. None change release state yet.
- **Known backlog observations (2026-09-13 smoke):** (1) first authenticated Analyze call returned HTTP 200 with an empty content stream; the designed local fallback engaged with honest messaging, immediate retry succeeded — transient, copy could distinguish "empty stream" from "unavailable". (2) keep-alive route design keeps the hidden YouTube iframe mounted across routes; pause-on-route-leave is implemented for both audio paths and deterministically covered for the `<audio>` and YouTube-player seams by the committed `e2e/youtube-route-lifecycle.spec.ts` regression. (3) replay-seek to a line start can transiently show no active line for ~1–2s during seek settle. None block release.
- **Lemma provenance claim (2026-09-10 journal): STALE / NOT A BUG.** Current code already maps MW headwords to `lemma_provenance: 'provider-confirmed'` server-side, forwards it through `fetchFromBackend` into the reference, and gates promotion via `isTrustedLemma`. Existing deterministic tests (`dictionaryService.test.ts` `cats→cat` provider-confirmed round-trip; `running` base_form exposure; `vocabularyEnrichment.test.ts` trust gating) pin the chain; focused rerun 26/26 PASS. No fix or dictionary refactor needed.
- **`DEEPSEEK_V4_MODEL_REFRESH_V1`: LOCAL VERIFIED (2026-09-13, not committed).** Official DeepSeek docs confirm `deepseek-v4-flash` / `DeepSeek-V4-Flash-0731`; routine EchoLearn calls disable default thinking with `thinking: {type:'disabled'}`. All three AI client services send `deepseek-v4-flash` + `thinking: {type:'disabled'}`; `api/ai.ts` forwards only that current model, falls stale/unknown model ids back to it, and forwards only the exact `{type}` thinking shape. `reasoning_effort` is not forwarded. Validation: focused `aiNodeHandler.test.ts` 10/10, full Vitest 578/578, `npx tsc -b` clean, ESLint clean on changed source/test files, `npm run build` PASS, `git diff --check` PASS. No real DeepSeek traffic was generated for this change; production verification is pending the next authorized deploy.
