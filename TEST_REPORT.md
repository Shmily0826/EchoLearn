...
- **Local-only work is ahead of Production.** Two local commits (`2ae29c2` study playback context, `4156bcc` bulk translation stale list) are **committed but not pushed**, so CI has not run on them and Production still serves `31d5f37`. The entries below describe the last pushed state; the 2026-09-16 sections describe the local evidence for the unpushed work.
