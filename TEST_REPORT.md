...
- **2026-09-22 - `ECHO-20260922-PROTOTYPE-KEY-CRASH-P0` Study route crash on a subtitle containing "constructor":** PRODUCTION VERIFIED. Reproduced against live Production, fixed, each guard falsified. `tsc -b` clean, lint 0 errors / 12 baseline warnings, Vitest **731/731** (+7), build PASS, local-audio + learner-friction E2E **13/13**. PR **#22** merged as **`b9ccc51`**, `deploy:check` PASS, and the same repro re-run on the live origin afterwards reported `crashed: false` with 2 transcript rows rendered (it reported `crashed: true`, 0 rows before the fix).
...
- **Gates actually run.** `npx tsc -b` clean; `npx eslint .` 0 errors / 12 warnings (recorded baseline); `npm test` **731/731**; `npm run build` PASS; `npx playwright test e2e/local-audio.spec.ts e2e/learner-friction-fix.spec.ts` **13/13**.
