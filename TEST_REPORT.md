...
- **Validation:** focused Vitest **6/6**; local-media browser E2E **5/5**; `tsc -b` PASS; lint **0 errors / 12 baseline warnings**; build PASS. `git diff --check` and final status are recorded after this documentation update.
- **Evidence boundary:** the 90 MB case is a size-contract test only. No real 90 MB browser/device run, GitHub commit/push, or Production deployment was performed.
