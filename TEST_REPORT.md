...
- **Result: PASS (local, uncommitted).** One new case in `e2e/study-ai-authenticated.spec.ts` — "a suggestion the aligner cannot place never becomes a @0:00 the learner can click" — replaces two isolated component guards with the real journey: `AIAnalysisPanel` → `localStorage` → `SentenceList` → `SentencesPage`.
...
- **Not covered.** CI, Production, `npm run build` and the emulator suite were not run this cycle — the change is test-only, so CI is the next authority and it will report 64 e2e cases against the `63 passed` line in `PROGRESS.md`. The player-side jump still has no automated assertion either: this case (like the existing seek cases) runs against stubbed third-party hosts with no real media, so the jump remains asserted only on Production via `npm run ai:seek-smoke`.
