...
- The minimal Worker source-record set is being pushed; the current frontend/Vercel and VPS candidates remain local and undeployed. Only the authorized Worker candidate was deployed. No secrets, subtitle bodies, ASR, Groq, or new provider were used.
...
- Current Vercel production is deployment `dpl_2NdAcrpPchWBjM4nLvRL27E4ngNg`, `READY`, from Git SHA `04cd7ab2832a0084f235e6a95daa505609ef66ec`; local `api/transcript.ts` and frontend service changes were not deployed. Current app and Worker health endpoints were HTTP 200.
