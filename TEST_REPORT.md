...
- **2026-09-22 - `ECHO-20260922-UI-POLISH-V1` Vocabulary page layout + Study material-entry compaction:** LOCAL VERIFIED only, on the dev server with a seeded guest library. Reproduced the learner's report in a headless browser at 2660x1420 dark, fixed four confirmed defects, and re-measured each. Gates: `tsc -b` clean, lint 0 errors / 12 baseline warnings, Vitest **718/718**, build PASS, affected E2E **22/22**, `git diff --check` clean. No provider traffic was generated (the dictionary tiers were intercepted in the reproduction probe). No merge, Production deployment or Production smoke is claimed. Full evidence below.
...
- **Result: LOCAL VERIFIED.** Four confirmed visual/layout defects fixed in `src/pages/VocabularyPage.tsx`, `src/components/LocalAudioImporter.tsx`, `src/i18n/translations.ts`.
...
- **Not covered.** No authenticated run (the seeded library is guest-local), no Production deployment or smoke, no mobile-PWA project run, and no real-device pass. The 2660x1420 reproduction is a synthetic viewport at `deviceScaleFactor: 1`, not the learner's actual display.
