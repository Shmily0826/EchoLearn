...
- **Not covered.** The `SentencesPage` chip is protected by the same rule but has no test of its own, and no E2E saves a sentence with `startTime: 0`. Also left alone: the seek control's `title` is a hardcoded English string shown in the Chinese UI — pre-existing, unrelated, out of scope.
