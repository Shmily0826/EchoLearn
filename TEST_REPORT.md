...
- **Failure path reproduced, not assumed.** The probe aborted `/api/dictionary`, `api.dictionaryapi.dev` and `api.datamuse.com` together; the page then showed exactly the learner's message, confirming it is the all-tiers-failed branch. Separately, live probes measured `api.dictionaryapi.dev` at **HTTP 522 after 19.5s** (origin down) while `https://echo-learn.uk/api/dictionary` returned 200 for 15 sample words — so the learner's original failure was an upstream/rate-limit window, not their setup. **Left unchanged on purpose:** the backfill fires batches of 4 with no delay against the endpoint's 120 req/min/IP limit, which a large library can exhaust by itself; widening that is a behavior decision, not a layout fix.
