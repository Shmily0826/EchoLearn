...
## CURRENT VALIDATION INDEX — 2026-09-12

- **Production accepted:** Dictionary reference consistency is **PRODUCTION ACCEPTED** on deployed exact SHA `5094c66c63018dd5a8cb0d39837528d0842c34c7`. The acceptance smoke used local `/api/dictionary` fixtures; paid/provider outbound traffic was `0`. This does not claim live provider billing or fresh provider traffic.
...
- **Current revalidation of the four older friction items:**
  - Audio silent-prewarm: **STALE / NOT ACTIVE**. Current `useAudioMode` has no old prewarm behavior; the focused current tests previously passed.
  - Guest onboarding versus AI: **RESOLVED / PREVENTED**. Onboarding is guest-accessible and the Study tour states that AI requires sign-in.
  - Failed/empty transcript zero-line session: **RESOLVED / PREVENTED** by the current load flow; generic storage capability is not a repro.
  - `undefined.toLowerCase`: **INSUFFICIENT EVIDENCE**. It was not reproduced on current HEAD and is not an active bug or patch recommendation.
- **Current open evidence-dependent work only:** Supadata cost/billing review, Invidious/Piped health and pruning, and low-priority consolidation leftovers. Real learning-session UX is a recommended future product-selection direction, not accepted implementation scope.
- **Other older friction observations:** invalid/unsupported URL feedback, dictionary-failure UX, login-form polish, and other 2026-09-08 observations remain **HISTORICAL / NOT CURRENTLY REVALIDATED**. They are not silently resolved and are not current commitments from this cleanup.
- **This docs-only revalidation did not run providers, network, browser, tests, build, lint, or Production traffic.** The classifications above are the current reconciliation boundary; use the dated entries below for raw historical evidence.
