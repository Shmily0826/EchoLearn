...
const StudyPage: React.FC = () => {
  ...
  const handleLoadVideo = useCallback(() => {
    ...
    const persistTranscriptInto = (
      sess: VideoStudySession,
      lines: TranscriptLine[],
      provenance?: { source?: string; diagnostics?: CaptionDiagnostics },
    ) => {
      const sLines = normalizeTranscriptToSentences(lines);
      setRawBlocks(lines);
      setSentenceLines(sLines);
      const updated = attachTranscriptToSession(sess, lines, sLines, Date.now(), provenance);
      saveCurrentSession(updated);
      setSession(updated);
    };
        ...
        runCaptionRequest(
          () => fetchBilibiliTranscript(bvid!, undefined, pg),
          {
            ...
            onSuccess: (res) => {
              const lines = res.lines;
              if (lines.length > 0) {
                persistTranscriptInto(fresh, lines, res);
                return {
                  count: lines.length,
                  source: res.source ?? null,
                  diagnostics: res.diagnostics,
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        <div className="study-layout flex flex-col lg:flex-row gap-4 lg:gap-6">
          ...
          <div className="w-full lg:w-[55%] flex-shrink-0">
            ...
            {/* Persisted caption provenance survives navigation/reload. */}
            {!fetchToast && session?.captionSource && rawBlocks.length > 0 && (
              <div
                className="mt-2 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 rounded-lg"
                data-testid="caption-diagnostics-saved"
                role="status"
              >
                {t('study.captionDiagnostics', { count: rawBlocks.length })}
                {' · '}
                {transcriptSourceLabel(t, platform, { source: session.captionSource })}
                {session.captionDiagnostics?.supadata?.attempted ? (
                  <span className="ml-1 text-amber-700 dark:text-amber-300">
                    {' · '}{t('study.supadataLikelyCredit')}
                    {' · '}{t('study.supadataBrowserEstimate', {
                      attempts: captionDiagnosticsAggregate.supadataAttempts,
                      credits: captionDiagnosticsAggregate.estimatedCredits,
                    })}
                  </span>
                ) : null}
              </div>
            ...
            {fetchToast && (
              <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-lg text-sm text-emerald-700 dark:text-emerald-300">
                ...
                <span>
                  ...
                  {fetchToast.diagnostics?.supadata?.attempted ? (
                    <span
                      className="ml-1 text-amber-700 dark:text-amber-300"
                      data-testid="caption-supadata-estimate"
                    >
                      {' · '}{t('study.supadataLikelyCredit')}
                      {fetchToast.diagnosticsAggregate ? (
                        <>
                          {' · '}{t('study.supadataBrowserEstimate', {
                            attempts: fetchToast.diagnosticsAggregate.supadataAttempts,
                            credits: fetchToast.diagnosticsAggregate.estimatedCredits,
                          })}
                        </>
                      ) : null}
                    </span>
