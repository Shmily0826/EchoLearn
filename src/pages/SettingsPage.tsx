...
const SettingsPage: React.FC<{ onLoginRequest?: () => void }> = ({ onLoginRequest }) => {
  ...
  const { user, logOut, resendVerificationEmail, deleteAccount } = useAuth();
  ...
  const openFeedback = useCallback(() => {
    ...
    if (!user.emailVerified) {
      // Unverified email accounts cannot submit feedback (see firestore.rules).
      setSyncMessage({ type: 'error', text: t('settings.verifyToFeedback') });
      return;
    }
    setFeedbackText('');
  ...
  const handleResendVerify = useCallback(async () => {
    ...
    try {
      ...
      setSyncMessage({ type: 'success', text: t('settings.verificationSent') });
    } catch {
      setSyncMessage({ type: 'error', text: t('settings.verificationFailed') });
    } finally {
      setResending(false);
    }
      ...
      {user && (
        <section className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm p-5 sm:p-6 mb-6">
          ...
          {user.emailVerified ? (
            ...
            <div className="mb-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-3 flex items-center justify-between gap-3">
              <div className="text-xs text-amber-700 dark:text-amber-300">
                <p className="font-medium">{t('settings.emailUnverified')}</p>
                <p className="opacity-80 mt-0.5">{t('settings.verifyHintSmall')}</p>
              </div>
              <button
                onClick={handleResendVerify}
                disabled={resending}
                className="shrink-0 px-3 py-1.5 text-xs font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-lg cursor-pointer disabled:opacity-60 whitespace-nowrap"
              >
                {resending ? t('settings.sending') : t('settings.resendVerify')}
              </button>
            </div>
          )}
          ...
          <div className="border-t border-gray-100 dark:border-slate-700 pt-4">
            ...
            <p className="text-[11px] text-gray-400 dark:text-gray-500 mb-3">
              {user.emailVerified ? t('settings.syncHint') : t('settings.verifyToSync')}
            </p>

            <div className="flex flex-wrap gap-2">
              <button
                onClick={handleSyncNow}
                disabled={fbSyncAction !== 'idle' || !user.emailVerified}
          ...
          <div className="border-t border-gray-100 dark:border-slate-700 pt-4 mt-4 flex items-center justify-between gap-3">
            <p className="text-xs text-gray-400 dark:text-gray-500">{t('settings.deleteAccountHint')}</p>
            <button
              onClick={handleDeleteAccount}
              className="shrink-0 px-3 py-1.5 text-xs font-medium text-red-500 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-50 dark:hover:bg-red-950 transition-colors cursor-pointer whitespace-nowrap"
            >
      ...
      <section className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm p-5 sm:p-6 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <button
            ...
            className="flex items-center gap-3 p-3 text-left border border-gray-100 dark:border-slate-700 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-gray-400 dark:text-gray-500 truncate">
                {user?.emailVerified ? t('feedback.subtitle') : t('settings.verifyToFeedback')}
              </p>
            </div>
          </button>
        </div>
