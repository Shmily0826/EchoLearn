...
/** Returns password strength: 0 = none, 1 = weak, 2 = medium, 3 = strong */
...
const LoginPage: React.FC = () => {
  ...
  const strengthLabel = pwStrength === 1
    ? t('login.pwWeak')
    : pwStrength === 2
      ? t('login.pwMedium')
      : pwStrength === 3
  ...
  const strengthColor = pwStrength === 1
    ? 'bg-red-500'
    : pwStrength === 2
      ? 'bg-amber-500'
      : pwStrength === 3
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      ...
      if (password !== confirmPw) {
        setError(t('login.errPwMismatch'));
        return;
      }
    }
  ...
  const inputClass =
    'w-full px-4 py-3 rounded-xl border text-sm outline-none transition-colors focus:ring-2 focus:ring-indigo-500/30';
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm rounded-2xl p-8 shadow-lg border relative"
        style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
      >
        {/* Language toggle */}
        <button
          onClick={toggleLang}
          className="absolute top-4 right-4 text-xs font-medium px-2.5 py-1 rounded-lg border transition-colors hover:bg-gray-100 dark:hover:bg-slate-700 cursor-pointer"
        ...
        <div className="flex items-center gap-3 my-6">
          ...
          <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{t('login.or')}</span>
          <div className="flex-1 h-px" style={{ backgroundColor: 'var(--color-border)' }} />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {[1, 2, 3].map((level) => (
                  <div
                    ...
                    className={`h-1 flex-1 rounded-full transition-colors ${
                      pwStrength >= level ? strengthColor : 'bg-gray-200 dark:bg-slate-700'
                    }`}
                  />
                ))}
              </div>
          ...
          {mode === 'signup' && (
            <input
              ...
              style={{
                backgroundColor: 'var(--color-input-bg)',
                borderColor: confirmPw && confirmPw !== password ? '#ef4444' : 'var(--color-border)',
                color: 'var(--color-text)',
              }}
            />
          )}
          ...
          <button
            type="submit"
            disabled={loading || !email || !password || (mode === 'signup' && (!confirmPw || password !== confirmPw))}
        ...
        <p className="text-center text-sm mt-6" style={{ color: 'var(--color-text-muted)' }}>
          ...
          <button
            onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setConfirmPw(''); }}
