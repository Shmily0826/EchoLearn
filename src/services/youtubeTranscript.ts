  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.lines && data.lines.length > 0) {
        data.source = data.source || 'local-proxy';
...
export class YouTubeTranscriptError extends Error {
  ...
  readonly diagnostics?: CaptionDiagnostics;
...
export class YouTubeAcquisitionBlockedError extends Error {
  ...
  constructor(diagnostics?: CaptionDiagnostics) {
...
export async function fetchYouTubeServerTranscript(
  ...
  onDiagnostics?: (diagnostics: CaptionDiagnostics) => void,
): Promise<TranscriptFetchResult | null> {
  ...
  const reportDiagnostics = (value: unknown): CaptionDiagnostics | undefined => {
    const diagnostics = normalizeCaptionDiagnostics(value);
    if (diagnostics) {
      recordCaptionDiagnostics(diagnostics);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data.lines) && data.lines.length > 0) {
          if (endpoint.label !== 'CF Worker') data.source = data.source || 'unknown';
          console.log(
        ...
        const diagnostics = reportDiagnostics(payload?.diagnostics);
        ...
        if (Object.values(TRANSCRIPT_ERROR_CODES).includes(code as TranscriptErrorCode)) {
          if (code === YOUTUBE_ACQUISITION_BLOCKED) throw new YouTubeAcquisitionBlockedError(diagnostics);
...
export interface TranscriptFetchResult {
  ...
  /** Provider/path provenance, e.g. supadata, vps, npm, youtube, or whisper. */
  source?: string;
  /** Optional privacy-safe diagnostics from the server fallback chain. */
  diagnostics?: CaptionDiagnostics;
}

function mergeResultDiagnostics(
  result: TranscriptFetchResult,
  previous?: CaptionDiagnostics,
): TranscriptFetchResult {
  const diagnostics = mergeCaptionDiagnostics(previous, result.diagnostics);
...
async function _fetchYouTubeTranscriptImpl(
  ...
  if (getLocalProxyUrl()) {
    try {
      ...
      if (localResult) return mergeResultDiagnostics(localResult, serverDiagnostics);
  ...
  try {
    ...
    const serverResult = await fetchYouTubeServerTranscript(videoId, lang, (detail) => {
      ...
      serverDiagnostics = mergeCaptionDiagnostics(serverDiagnostics, diagnostics);
    });
    if (serverResult) return mergeResultDiagnostics(serverResult, serverDiagnostics);
    if (options.allowAsr) {
    ...
    if (err instanceof YouTubeAcquisitionBlockedError) throw err;
    if (err instanceof YouTubeTranscriptError) {
      serverDiagnostics = mergeCaptionDiagnostics(serverDiagnostics, err.diagnostics);
    }
    if (
