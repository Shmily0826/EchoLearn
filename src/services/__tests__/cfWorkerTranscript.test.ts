...
describe('CF Worker transcript routing contract', () => {
  ...
  it('exposes the cache state header to browser JavaScript at the CORS boundary', async () => {
    const globals = globalThis as unknown as { caches?: unknown };
    const originalCaches = globals.caches;
    const originalFetch = globalThis.fetch;
    ...
    globals.caches = {
      default: { match: vi.fn(async () => cacheResponse), put: vi.fn() },
    };
    globalThis.fetch = vi.fn(() => Promise.reject(new Error('provider should not be called'))) as typeof fetch;

    try {
      const response = await worker.fetch(new Request(
        'https://worker.test/api/transcript?videoId=cache-cors&lang=en',
        { headers: { Origin: 'https://echo-learn.uk', 'cf-connecting-ip': 'synthetic-cache-cors' } },
      ), {});
      const exposed = (response.headers.get('Access-Control-Expose-Headers') ?? '')
        .split(',')
        .map((header: string) => header.trim());

      expect(response.headers.get('Access-Control-Allow-Origin')).toBe('https://echo-learn.uk');
      expect(response.headers.get(TRANSCRIPT_CACHE_HEADER)).toBe('HIT');
      expect(exposed).toEqual(expect.arrayContaining([
        'X-EchoLearn-Trace-Id',
        TRANSCRIPT_CACHE_HEADER,
      ]));
      expect(await response.json()).toMatchObject({ lines: [{ text: 'cached' }] });
    } finally {
      globalThis.fetch = originalFetch;
      if (originalCaches === undefined) delete globals.caches;
      else globals.caches = originalCaches;
  ...
  it('proves one caption request transitions from MISS to HIT at the response boundary', async () => {
    const originalFetch = globalThis.fetch;
    const globals = globalThis as unknown as { caches?: unknown };
    const originalCaches = globals.caches;
    const entries = new Map<string, Response>();
    const match = vi.fn(async (request: Request) => entries.get(request.url)?.clone() ?? null);
    const put = vi.fn(async (request: Request, response: Response) => {
      entries.set(request.url, response.clone());
    });
    const providerFetch = vi.fn((input: RequestInfo | URL) => {
        ...
        return Promise.resolve(new Response(JSON.stringify({
          ...
          captions: {
            playerCaptionsTracklistRenderer: {
              captionTracks: [{
                baseUrl: 'https://www.youtube.com/api/timedtext?v=fresh-caption-control',
                languageCode: 'en',
              }],
            },
          },
      ...
      if (requestUrl.includes('timedtext')) {
        return Promise.resolve(new Response(
          '<transcript><text start="0" dur="1">fresh caption control</text></transcript>',
          { status: 200 },
        ));
      }
    ...
    const makeRequest = () => new Request(
      'https://worker.test/api/transcript?videoId=fresh-caption-control&lang=en',
      { headers: { Origin: 'https://echo-learn.uk', 'cf-connecting-ip': 'synthetic-miss-hit' } },
    );

    try {
      const first = await worker.fetch(makeRequest(), {});
      const providerCallsAfterMiss = providerFetch.mock.calls.length;
      const second = await worker.fetch(makeRequest(), {});

      expect(first.status).toBe(200);
      expect(second.status).toBe(200);
      expect(first.headers.get(TRANSCRIPT_CACHE_HEADER)).toBe('MISS');
      expect(second.headers.get(TRANSCRIPT_CACHE_HEADER)).toBe('HIT');
      expect(first.headers.get('Access-Control-Expose-Headers')).toContain(TRANSCRIPT_CACHE_HEADER);
      expect(second.headers.get('Access-Control-Expose-Headers')).toContain(TRANSCRIPT_CACHE_HEADER);
      expect(await first.json()).toMatchObject({ lines: [{ text: 'fresh caption control' }] });
      expect(await second.json()).toMatchObject({ lines: [{ text: 'fresh caption control' }] });
      expect(match).toHaveBeenCalledTimes(2);
      expect(put).toHaveBeenCalledTimes(1);
      expect(providerFetch.mock.calls.length).toBe(providerCallsAfterMiss);
    } finally {
      globalThis.fetch = originalFetch;
      if (originalCaches === undefined) delete globals.caches;
      else globals.caches = originalCaches;
  ...
  it('preserves an explicit cache bypass marker for ASR and diagnostic responses', () => {
    const asrResponse = withCors(
      new Response(JSON.stringify({ error: 'asr_required' }), {
        status: 409,
        headers: { [TRANSCRIPT_CACHE_HEADER]: 'BYPASS' },
      }),
      'https://echo-learn.uk',
    );

    expect(asrResponse.headers.get(TRANSCRIPT_CACHE_HEADER)).toBe('BYPASS');
    expect(asrResponse.headers.get('Access-Control-Expose-Headers')).toContain(TRANSCRIPT_CACHE_HEADER);
  });
  ...
  it('writes only a usable caption result to the edge cache', async () => {
    ...
    try {
      ...
      expect(result.status).toBe(200);
      expect(result.headers.get('X-EchoLearn-Transcript-Cache')).toBe('MISS');
      expect(put).toHaveBeenCalledTimes(1);
  ...
  it('keeps the dedicated subtitle evaluator dormant in normal transcript routing', async () => {
    const originalFetch = globalThis.fetch;
    const globals = globalThis as unknown as { caches?: unknown };
    const originalCaches = globals.caches;
    const calls: string[] = [];
    globals.caches = { default: { match: vi.fn(async () => null), put: vi.fn() } };
    globalThis.fetch = vi.fn((input) => {
    ...
    try {
      await handleTranscript(
        new URL('https://worker.test/api/transcript?videoId=dQw4w9WgXcQ&lang=en'),
        { SCRAPE_API_KEY: 'synthetic-test-key' },
        'dormant-provider-trace',
      );

      expect(calls.some((call) => {
        try {
          return new URL(call).pathname === '/api/v1/youtube/subtitles';
        } catch {
          return false;
        }
      })).toBe(false);
...
describe('CF Worker caption deadline behavior', () => {
  ...
  it('routes explicit ASR directly and preserves VPS acquisition blocking', async () => {
    ...
    try {
      ...
      expect(result.headers.get(TRANSCRIPT_CACHE_HEADER)).toBe('BYPASS');
