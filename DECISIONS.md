...
- **Release boundary:** LOCAL VERIFIED. Commit `ea9bd3e` is pushed and PR #16 is open. The GitHub `test` job and Vercel check passed, but the full E2E check fails in the pre-existing, out-of-scope dictionary-popup test (`learner-friction-fix.spec.ts`, 96 passed / 4 skipped / 1 failed on two runs); no merge, Production deploy, or Production smoke has occurred.
