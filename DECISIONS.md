...
- **Supersedes:** None recorded.
- **Owner action 2026-09-24:** the comment toggle under **Project → Settings → Git → Connected Git Repository** was switched off in the Vercel dashboard, as recommended. The `deployment_status` events toggle was left alone for now. The next pull request is the observation that confirms the comment is gone without costing the `Vercel` merge gate; its result is recorded on that PR and appended here.
