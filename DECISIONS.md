...
## ECHO-20260923-RECORDING-NOISE-V1 - Vercel stays a gate, not a commenter; acceptance records ride along

- **Status:** ACTIVE.
- **Decision:** `"github": { "silent": true }` is set in `vercel.json`, so Vercel no longer posts a deployment comment on every push or pull request. The `Vercel` and `Vercel Preview Comments` **status checks are unaffected and remain the merge gate** — the comment and the check are separate features, and only the comment is silenced. Related: an implementation change and its acceptance record (`PROGRESS.md` / `TEST_REPORT.md` / this file) ship in **one** pull request; a separate docs-only PR is opened only when the record has to land independently of the code it describes.
- **Rationale:** A ten-pull-request session (PR #18-#27, half of them split-out records) produced one vercel[bot] "left a comment" email per PR, and the comment body is a deployment status table repeating what the checks already say. The repository owner's inbox is a shared signal channel; a change that removes ten mails per campaign and no information is worth more than the convenience of the bot's summary. The genuine failure notices (`PR run failed: CI`) are a different category and were kept — two of them pointed at real breakage.
- **Guardrails:** Silencing the comment must not be confused with disabling deployment or the checks; confirm on the next pull request after this one lands that the Vercel checks still report and that no vercel[bot] comment appears. Do not widen this to GitHub's own notification settings — that is the owner's account-level choice, and the API path needs a `notifications` scope the current credential does not have.
- **Supersedes:** None recorded.
