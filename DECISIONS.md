...
- **Status:** ACTIVE / PRODUCTION VERIFIED. The browser-local V2 local-media path accepts `.mp3`, `.m4a`, and `.wav` up to **200 MiB** so a roughly 90 MB NotebookLM podcast can be paired with an existing `.srt` or `.vtt` without upload or transcription.
...
- **Release boundary:** PR #16 was completed with implementation commit `ea9bd3e` and regression-harness fix `c75b52d`; CI (`test`, `e2e`) and Vercel checks passed, and it merged to `main` as **`04366f8fff5b6190255e5873ae8f4fc4f0485715`**. `npm run deploy:check -- --expect 04366f8` passed: that merge commit is the newest successful Production deployment. A provider-free Production smoke verified the 200 MiB UI boundary, small WAV+SRT browser-local import with zero `/api/audio-transcribe` requests, refresh Blob restoration, and the 390x844 dictionary popup in both loading and loaded states. The only remaining acceptance is Android real-device selection of the roughly 90 MB NotebookLM audio plus SRT/VTT.

## ECHO-20260922-UI-POLISH-V1 - Reproduce a layout report before changing classes

- **Status:** ACTIVE.
- **Decision:** A learner-reported layout or contrast defect is reproduced in a real browser at the reported viewport before any class is edited, and the fix is re-measured in the same conditions. Geometry numbers (`getBoundingClientRect`, `getComputedStyle`), not class names, are the evidence. A user-visible string that is not routed through `t()` counts as a defect in its own right, independent of whether the layout complaint was valid.
- **Rationale:** All four Vocabulary defects were invisible in the source and obvious only once measured: the 1024px island at a 2660px window, the grid's default `stretch` turning short cards into empty boxes, and two `w-full` status messages breaking a `flex-wrap` action bar into a ragged stack. Conversely `flex flex-col` silently stretched an `inline-block` part-of-speech pill across the card — a regression that only a screenshot could catch.
- **Guardrails:** Never seed a synthetic `localStorage` library into the learner's own browser profile; it would overwrite real data. The Qoder in-app browser cannot be screenshotted (it reports `NATIVE_BROWSER_VIEWPORT_UNAVAILABLE` with a 0x0 viewport), so use a headless Playwright context against the dev server with `page.route` interception on every dictionary/provider tier so the run costs nothing. Do not trade away an existing above-the-fold discovery assertion to reclaim space — the Study material entry was compacted to one row precisely so `study-material-entry-campaign.spec.ts` keeps passing unchanged.
- **Supersedes:** None recorded.


## ECHO-20260923-RECORDING-NOISE-V1 - Vercel PR-comment noise lives in the dashboard, and records ride along

- **Status:** ACTIVE.
- **Decision:** Two things. First, an implementation change and its acceptance record (`PROGRESS.md` / `TEST_REPORT.md` / this file) ship in **one** pull request; a docs-only PR is opened only when the record must land independently of the code. Second, the noise from Vercel's pull-request comments is **not** addressable from this repository: it is a project setting, and silencing it is the owner's call in the Vercel dashboard.
- **What was tried and did not work, recorded so nobody retries it:** `"github": { "silent": true }` in `vercel.json` was proposed to stop the per-deployment `vercel[bot]` comment. Observed on PR #28, whose own branch deployment carried the flag: **the comment still appeared** and the `Vercel Preview Comments` check still reported. Vercel's own documentation now names `github.silent` **deprecated**, with the live control under **Project → Settings → Git → Connected Git Repository** (comment toggles) and a separate **`deployment_status` events** toggle that stops the deployment entries in the PR activity log. Neither is reachable from here: they are account-level, and the notifications API needs a scope this credential does not have.
- **Rationale:** A ten-pull-request session (PR #18-#27) produced one `vercel[bot]` "left a comment" email per PR, repeating a deployment status table the checks already carry. The genuine `PR run failed: CI` notices were kept on purpose — two of them pointed at real breakage, so reducing mail must not cost that signal. Shipping a deprecated key that observably does nothing would have added a config line whose comment contradicts reality.
- **Guardrails:** If the dashboard toggles are used later, confirm on the next pull request that the `Vercel` check still reports — it is the merge gate, and `deployment_status` must not be disabled while anything depends on it. This repository currently has **no** workflow referencing `deployment_status` or Vercel events, so that toggle is safe here today; re-check before using it if a workflow ever starts depending on deployment events.
- **Supersedes:** None recorded.
- **Owner action 2026-09-24, with the observation:** the comment toggle under **Project → Settings → Git → Connected Git Repository** was switched off, and PR #29 became the check. Result, against PR #28 immediately before it: the `vercel[bot]` comment went from 1 to **0**, and the `Vercel` deployment check still **passes** — so the merge gate survived and the mail source did not. One consequence worth knowing: the companion **`Vercel Preview Comments` check disappeared from the required set as well** (4 checks before, 3 after). That is harmless here because the repository sets no branch protection requiring it, but if branch protection is ever added, do not list a check that no longer reports. The separate `deployment_status` events toggle was deliberately left alone: it only affects the PR activity log, and the noise it removes is not the mail.

## ECHO-20260924-SELF-TRIGGERED-GUARD-V1 - Never read your own output as user intent, and never trust an unverified edit

- **Status:** ACTIVE.
- **Decision (1):** When an effect both writes to a channel and listens on that same channel to decide whether the *user* acted, the effect suppresses itself. Detect intent from input events (`wheel`, `touchmove`, key presses), not from the output the effect produces (`scroll`, `change`, `input` fired programmatically). The transcript follow-scroll did exactly this and shipped broken for as long as the local-media path existed.
- **Rationale:** The suppression window (3 s) was longer than the cue cadence it was reacting to (1.76 s), so every automatic jump cancelled the next one-to-two follows and the list fell permanently behind the highlighted line. It passed every existing test because the desktop container is tall enough that the drift needs ~13 cues to leave the screen — a real user sees it in seconds, and an assertion that only checks "is the row on screen right after one jump" cannot see it at all.
- **Decision (2):** A scripted edit used to falsify a test must be **verified to have landed** (grep the file, or assert the replacement count) before its test result is read as evidence.
- **Rationale:** Three times in one session a nearly-wrong conclusion came from an edit that silently did not apply: temporary reverts written with `\n` patterns against CRLF working copies, and a `node -e` swap that matched nothing and reported success. On the follow-scroll case the first falsification "passed" against code that had never been reverted, which would have been recorded as "the guard has teeth" when it had none.
- **Guardrails:** When a behavior is a *rate* rather than a *state* (does the list follow every line?), assert the rate over a sequence, not a single sample — a one-shot assertion cannot distinguish 1-in-3 from 1-in-1. And a synthetic fixture must satisfy the property under test: a 40 s WAV behind a 2557 s subtitle measured nothing, and a 1000 Hz WAV is rejected outright by Chromium (`MEDIA_ERR_SRC_NOT_SUPPORTED`, `duration: NaN`) where 8000 Hz loads.
- **Supersedes:** None recorded.

## ECHO-20260924-ASYNC-DEP-V1 - An effect must not bail permanently on a value it never re-reads, and a test must enter the way a user enters

- **Status:** ACTIVE.
- **Decision (1):** A React effect's guard may test only its **declared dependencies**. A mutable ref read in the guard (`!playerRef.current`) is a value the effect will never re-run for, so failing on it converts a transient startup ordering into a permanent dead end — the effect simply never runs again. Tolerate the missing collaborator *inside* the recurring work (the per-tick `if (!ref.current) return`), where it costs one beat, not at the gate that decides whether the work ever exists.
- **Rationale:** `usePlaybackPosition` created its 100 ms polling interval only if the player handle already existed. On a reopened lesson the audio source is a blob URL that resolves after that effect has run, so the clock never started: `currentTime` stayed 0, no transcript line was ever highlighted, and the follow-scroll — which fires on an index *change* — never ran, while the player's own progress bar moved on because `AudioPlayer` renders it from its internal `timeupdate` state. It presented as "the subtitles cannot scroll", and the two fixes spent on the scroll logic beforehand were aimed at the wrong layer.
- **Decision (2):** When a defect is reported against a feature that can be entered two ways, the regression test must use the way **the reporter entered it**. For anything persisted, that means a reload / restore / re-mount, not a fresh in-memory setup.
- **Rationale:** Every one of the 965 tests that touched transcript scrolling drove a freshly imported lesson, where the ordering happens to work. The reopen path had zero coverage, so the bug survived two fix attempts and a full suite run each time.
- **Guardrails:** Before changing a layer, measure whether that layer is the one that is broken. Here the learner's own session was read directly (`localStorage.echolearn_session` from the live tab) to answer "is the timeline wrong?" — 1161 cues -> 591 sentences, 0 -> 2571 s, a single 0.56 s gap — and their real cue timings were replayed against Production at their real viewport before any code was touched. The fresh-import run passed on Production, which is what pointed at the entry path rather than at the algorithm. Also: a UI state that is rendered from a component's private clock can disagree with the app-level clock; when two consumers of "the time" disagree, suspect the plumbing before the algorithm.
- **Supersedes:** None recorded.
- **Reinforced the same day, second instance:** the same shape sat in `TranscriptViewer`'s listener-attach effect — `if (!activeLineRef.current) return;` inside an effect keyed on `[handleUserScroll, lines]`. A real Whisper subtitle starts at 0.07 s, so there is no active row when `lines` first arrives, the `wheel` / `touchmove` listeners were never attached, and the component could not notice a learner scrolling at all. Every fixture started its first cue at 0, which mounts an active row and reaches the pane through it. The pair rule with `ECHO-20260924-SELF-TRIGGERED-GUARD-V1`: a guard on a value that is not a dependency needs either that value as a dependency or a source that is always present (the component's own root), **and** a fixture must be checked for whether it passes the guard by accident.

## ECHO-20260924-ABSENCE-VS-FAILURE-V1 - "Not found" and "could not ask" are different answers, and a heuristic list must be re-measured

- **Status:** ACTIVE.
- **Decision (1):** A lookup path that consults several tiers must keep three outcomes separate — an answer, a **confirmed absence**, and **no answer** — and only the last may surface as "service unavailable". `OR`-ing failures across tiers (`failed = freeDict.failed || datamuse.failed`) silently converts the first into the second whenever any tier is sick.
- **Rationale:** `api.dictionaryapi.dev` spent today returning 522 after ~20 s for every word. With the OR rule, every brand, name and coinage in a transcript ("EchoLearn") read as *Dictionary service unavailable. Please try again.* while the backend had already answered 404 — a complete and correct answer. The message also told the learner to retry something that will never succeed.
- **Decision (2):** Authority to confirm absence belongs only to a tier that could actually have found the word. The backend qualifies (it consults Merriam, Free Dictionary and Datamuse itself); the small free dictionary does not — it does not define "cat" either — and neither does an empty Datamuse result, which is a word-association index.
- **Rationale:** The first version of this change marked every tier's 404 authoritative, and **two pre-existing tests went red** for the right reason: they pin that a malformed or failed pipeline must not be reported as a confident "no entry". The tests were correct and the change was wrong; the fix was to narrow the authority, not to relax the tests (one fixture was moved from `backend 404` to `backend 502` only because its subject is a malformed body, not a miss).
- **Decision (3):** When a blocklist or allowlist encodes an empirical claim ("the free dictionaries will never define these"), re-measure the claim against the provider before reasoning from the list. The failure mode is not a missing answer, it is a **confidently wrong** one.
- **Rationale:** The claim was false for every one of the 63 tokens except `tbh` and `kmh` — and the interesting part is what came back instead: `china` → "a hard white material made of baked clay", `trump` → "a card from the suit…", `musk` → "a strong-smelling substance used in perfume", `facebook` → "a reference book containing photographs", `btw` → "Baoding Tianwei Baobian Electric Co., Ltd.", `idk` → a biography of the rapper IDK, `rn` → "registered nurse". A learner clicking `ai` got nothing at all while Merriam was sitting on "artificial intelligence". The working rule is *would the returned gloss name the thing the learner clicked?* — and the measured glosses now live in `src/utils/properNouns.ts` so the next reader inherits the evidence rather than the guess.
- **Guardrails:** Report the classification to the learner in the same words you would use in a ticket: "no entry" for an absence, "unavailable" only for a failure to ask. Note the coarse edge that remains: a capitalised coinage mid-sentence now reads "Dictionary entry not found", which is honest but less kind than the name/brand line; recognising one would need a heuristic the popup does not have today.
- **Supersedes:** None recorded.

## ECHO-20260925-1025 - local sync, accessibility, and media URL cleanup

- Study vocabulary and sentence save feedback confirms the local save only. Background sync failures persist the pending marker; Settings retains manual sync and status feedback, and logout protection remains. No sync copy or behavior change was needed.
- Local audio URLs are revoked only after the IndexedDB delete transaction completes; an aborted delete retains the playback URL. All registered URLs are revoked after successful whole-database deletion. Leaving Study already pauses the shared player handle.
- Added a localized keyboard skip link with a focusable main target, a dark-mode accessible name, and status semantics for the translation warning.
