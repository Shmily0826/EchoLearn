...
def _groq_transcribe(
    ...
    *,
    ...
    upload_content_type: str = "audio/mpeg",
    ...
    _trace_event("audio_file_ready", sizeBytes=len(blob), format=upload_content_type)
    ...
    if require_timed_segments and not lines:
        raise HTTPException(
            status_code=422,
            detail={"code": "no_speech", "message": "Whisper returned no timed speech segments"},
        )
...
LOCAL_AUDIO_MAX_BYTES = 25 * 1024 * 1024
LOCAL_AUDIO_CONTENT_TYPES = {
...
async def audio_transcribe(request: Request):
    """Transcribe a browser-uploaded local audio file with timed segments."""
    if not GROQ_API_KEY:
        raise HTTPException(status_code=503, detail="ASR not configured (no GROQ_API_KEY)")
    if not YTDLP_API_KEY:
        raise HTTPException(status_code=503, detail="Local audio ASR not configured")
    if request.headers.get("X-Api-Key") != YTDLP_API_KEY:
    ...
    allowed_types = LOCAL_AUDIO_CONTENT_TYPES.get(extension)
    if not allowed_types or content_type not in allowed_types:
        raise HTTPException(status_code=415, detail={"code": "invalid_audio", "message": "Use an mp3, m4a, or wav audio file"})

    blob = await upload.read(LOCAL_AUDIO_MAX_BYTES + 1)
    if not blob:
        raise HTTPException(status_code=422, detail={"code": "invalid_audio", "message": "Audio file is empty"})
    if len(blob) > LOCAL_AUDIO_MAX_BYTES:
        raise HTTPException(status_code=413, detail={"code": "audio_too_large", "message": "Audio files must be 25 MiB or smaller"})

    with tempfile.TemporaryDirectory() as td:
        path = os.path.join(td, f"upload{extension}")
        with open(path, "wb") as fh:
            fh.write(blob)
        return JSONResponse(_groq_transcribe(
            path,
            upload_name=filename,
            upload_content_type=content_type,
            require_timed_segments=True,
        ))
